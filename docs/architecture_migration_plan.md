...
The production caller audit classified the parallel RockMassProfile/RockStructure/BlastDesign/DrillingPattern/ChargeSegment/BlastExecution/WallAssessment graph, its Lithology and old ExplosiveType support tables, the Mine/BlastBlock compatibility graph, and the old directly Block-owned Attachment path as DEAD. The consolidated `0001_mvp_baseline` contains only the current schema. Revisioned BlastEvent Technical Card persistence is the sole canonical engineering path, AssessmentEntityAttachment remains the canonical one-owner attachment path, and Blast Event workflow status is derived rather than persisted.
