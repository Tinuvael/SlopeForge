...
## Replace-репозиторий полного состояния

`AssessmentStateRepository` хранит целиком текущее состояние одного Site. Запись
намеренно работает по схеме **replace**: прежний `AssessmentWorkspace` со всеми
Assessment-дочерними строками удаляется каскадно, затем в той же транзакции
создаётся новый workspace и весь переданный агрегат. Целочисленные ID строк БД
поэтому одноразовые и могут измениться, а строковые domain ID остаются стабильными.

Здесь нет инкрементальной синхронизации, optimistic locking, сравнения истории
или сохранения старых Assessment-строк. Метаданные вложений пересоздаются, но
физические файлы не читаются, не копируются и не удаляются. Связи BlastEvent с
BlastBlock пока не сохраняются (`blast_block_id` новых событий равен NULL), а
существующие Mine, Site и BlastBlock остаются нетронутыми.

Это намеренное ограничение однопользовательского этапа с тестовыми данными. До
промышленной или совместной работы могут понадобиться инкрементальные обновления,
контроль конкурентного доступа и неизменяемость исторических ревизий.
...
уникальность `domain_id`; то же правило действует для сущностей оценки и вложений, у которых
нет собственной колонки `workspace_id`. Родительская цепочка всё равно однозначно определяет
workspace. Это оставляет будущему импортёру возможность точно сохранить `D-001` и прочие ID.
...
Обязательные поля существующего `EntityAttachment` (`subtype`, `custom_subtype`, `title`, дата,
описание, MIME-тип и размер) имеют `NOT NULL`. Пустая строка допустима там, где её использует
текущий домен, но отсутствующее значение `NULL` не допускается.
