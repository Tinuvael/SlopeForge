...
class BenchSectionDraft:
    ...
    comment: str = ""
    status: str = "ready"
    upper_boundary_is_assessment: bool = True

    def __post_init__(self) -> None:
        # Старые файлы имели статус оценки. Для геометрии они считаются готовыми.
    ...
    def assessment_segment_ids(self) -> list[str]:
        return ([self.upper_segment_id] if self.upper_boundary_is_assessment else []) + list(self.intermediate_segment_ids)
