...
def test_expected_alembic_head_resolves_real_repository_graph():
    ...
    assert repository_heads == ["0005_explosive_charge_form"]
    assert startup._expected_alembic_head() == repository_heads[0]


class FakeScript:
    def get_revision(self, revision):
        return object() if revision in {"0001_mvp_baseline", "0002_workflow_status", "0003_explosive_catalog", "0004_charge_presets", "0005_explosive_charge_form"} else None
