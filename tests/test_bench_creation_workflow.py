...
def test_confirm_current_step_reports_only_valid_workflow_action():
    controller = BenchCreationController()
    controller.start_bench()
    assert controller.confirm_current_step() is None
    controller.choose_line("LOW")
    assert controller.confirm_current_step() == "use_line"
    controller.use_selected_line(); controller.add_marker("LOW", 1); controller.add_marker("LOW", 5)
    assert controller.confirm_current_step() == "confirm_segment"


def test_line_candidates_cycle_and_clear_after_confirmation():
    controller = BenchCreationController(); controller.start_bench()
    controller.set_line_candidates(["A", "B", "C"], 10, 20)
    assert controller.current_candidate() == "A"
    assert controller.next_candidate() == "B"
    assert controller.previous_candidate() == "A"
    controller.use_selected_line()
    assert controller.active_line_id == "A"
    assert controller.candidate_line_ids == []
