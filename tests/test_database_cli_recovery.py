...
def _settings(name):
    return Settings(f"postgresql+psycopg://owner:secret@db.example:5432/{name}", Path("/tmp/storage"))


def test_migrate_unknown_revision_is_controlled(monkeypatch, capsys):
    monkeypatch.setattr(cli.command, "upgrade", lambda *_: (_ for _ in ()).throw(
...
def test_reset_refuses_protected_databases(monkeypatch, capsys):
    for name in ("postgres", "template0", "template1"):
        monkeypatch.setattr(cli.Settings, "from_env", lambda name=name: _settings(name))
...
def test_reset_wrong_confirmation_aborts_before_connection(monkeypatch, capsys):
    monkeypatch.setattr(cli.Settings, "from_env", lambda: _settings("slopeforge"))
    monkeypatch.setattr(cli, "create_engine", lambda *_a, **_k: (_ for _ in ()).throw(
...
def test_reset_exact_confirmation_recreates_and_migrates(monkeypatch):
    ...
    class Engine:
        ...
        def dispose(self): calls.append(("dispose", None))

    monkeypatch.setattr(cli.Settings, "from_env", lambda: _settings("slopeforge"))
    monkeypatch.setattr(cli, "create_engine", lambda url, **kwargs: Engine())
    monkeypatch.setattr(cli.command, "upgrade", lambda config, target: calls.append(("upgrade", target)))
