"""remove duplicate legacy engineering persistence
...
LEGACY_ENUMS = (structure_type, drilling_role, charge_segment_type, wall_rating, attachment_kind)
...
def upgrade() -> None:
    # PostgreSQL keeps enum types after their columns are dropped, so remove
    # dependent tables first and then explicitly remove the unused types.
    for table_name in (
        "charge_segments", "drilling_patterns", "blast_designs",
        "rock_structures", "rock_mass_profiles", "blast_executions",
        "wall_assessments", "attachments", "explosive_types", "lithologies",
    ):
        op.drop_table(table_name)
    bind = op.get_bind()
    for enum in reversed(LEGACY_ENUMS):
        enum.drop(bind, checkfirst=True)

def downgrade() -> None:
    # Recreate enum types before the 0010 tables that use them.
    bind = op.get_bind()
