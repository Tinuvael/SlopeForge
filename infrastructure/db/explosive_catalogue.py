...
def _domain(row: ExplosiveProductRow) -> ExplosiveProduct:
    return ExplosiveProduct(
        ...
        charge_form=ChargeForm(row.charge_form), explosive_class=ExplosiveClass(row.explosive_class),
        cartridge_length_mm=float(row.cartridge_length_mm) if row.cartridge_length_mm is not None else None,
    )
...
def _values(product: ExplosiveProduct) -> dict:
    return dict(name=product.name, kind=product.kind.value,
                ...
                charge_form=product.charge_form.value,
                explosive_class=product.explosive_class.value,
                cartridge_length_mm=product.cartridge_length_mm,
