...
class AuditLogRepository:
    ...
    def list_for_entity(self, entity_type: str, entity_id: str, limit: int = 200) -> list[AuditLogEntryRow]:
        with self.session_factory() as session:
            entries = list(session.scalars(
                select(AuditLogEntry)
                .options(joinedload(AuditLogEntry.user))
                .where(AuditLogEntry.entity_type == entity_type,
    ...
    def list_for_blast_event(self, event_id: str, limit: int = 200) -> list[AuditLogEntryRow]:
        return self.list_for_entity("blast_event", event_id, limit)
