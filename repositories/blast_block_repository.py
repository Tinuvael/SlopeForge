...
class BlastBlockRow:
    ...
    horizon_m: Decimal | None; planned_blast_date: date | None; status: str; author_name: str | None; created_at: datetime; updated_at: datetime; comment: str | None; created_by_user_id: int | None

class BlastBlockRepository:
    def __init__(self, session_factory: Callable[[], Session]): self.session_factory = session_factory
    def list_blocks(self, number_query=None, domain_id=None, site_id=None, status=None, **_ignored):
    ...
    def _to_row(block):
        ...
        return BlastBlockRow(block.id, block.block_number, block.domain_id, block.domain.name, site.id, site.name, site.mine_id, site.mine.name, block.horizon_m, block.planned_blast_date, block.status, author, block.created_at, block.updated_at, block.comment, block.created_by_user_id)
