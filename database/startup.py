...
def initialize_database_runtime():
    try:
        ...
        if missing:
            raise StartupError(
                "В PostgreSQL не найдены нужные таблицы: " + ", ".join(missing[:8]) + ("..." if len(missing) > 8 else ""),
        ...
        try:
            ...
            server = None
        raise StartupError("Не удалось подключиться к PostgreSQL или проверить таблицы.", server) from exc
