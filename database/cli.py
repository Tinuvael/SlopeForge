...
def prepare_db() -> int:
    try:
        ...
        if settings.is_sqlite_dev:
            ...
            print("SQLite development schema prepared with Base.metadata.create_all(). Do not use it for production data.")
            return 0
...
def migrate() -> int:
    ...
    if settings.is_sqlite_dev:
        engine = create_database_engine(settings)
        Base.metadata.create_all(engine)
        print("SQLite development schema prepared with Base.metadata.create_all(). Alembic is reserved for PostgreSQL.")
        return 0
...
def migration_status() -> int:
    settings = Settings.from_env()
    if settings.is_sqlite_dev:
        print("sqlite-dev mode uses Base.metadata.create_all() for local development; Alembic status applies to PostgreSQL mode.")
        return 0
...
def init_app() -> int:
    try:
        ...
        if Settings.from_env().is_sqlite_dev:
            Base.metadata.create_all(engine)
        else:
            command.upgrade(alembic_config(), "head")
