...
def create_first_admin_with_lock(session: Session, username: str, password: str, full_name: str | None = None) -> User:
    """Create the first admin safely for the current database mode.

    PostgreSQL uses a transaction-scoped advisory lock. sqlite-dev uses only a
    local in-process lock; it is convenient for development, but it is not a
    protection for several computers or several independent application
    processes.
    """
    bind = session.get_bind()
    if bind.dialect.name == "postgresql":
        session.execute(text("SELECT pg_advisory_xact_lock(hashtext(:lock_key))"), {"lock_key": FIRST_ADMIN_LOCK_KEY})
        return _create_first_admin_after_lock(session, username, password, full_name)
    if bind.dialect.name == "sqlite":
        with _sqlite_first_admin_lock:
            return _create_first_admin_after_lock(session, username, password, full_name)
    return _create_first_admin_after_lock(session, username, password, full_name)


def _create_first_admin_after_lock(session: Session, username: str, password: str, full_name: str | None = None) -> User:
    existing_count = session.scalar(select(func.count(User.id))) or 0
    if existing_count > 0:
        raise FirstAdminAlreadyExistsError("Первый администратор уже создан. Войдите под существующим пользователем.")
