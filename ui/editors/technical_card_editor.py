...
class TechnicalCardDialog(QDialog):
    def __init__(self, event, card, revision, save_callback, parent=None, read_only=False):
        ...
        self.setWindowTitle(f"Technical Card — {event.name}"); self.setMinimumSize(760, 560); self.resize(940, 720)
        root = QVBoxLayout(self); meta = QLabel(f"BlastEvent ID: {event.id}   |   Geometry revision: {revision.geometry_revision_id}")
        ...
        self.draft_button = QPushButton("Save draft"); self.complete_button = QPushButton("Complete"); cancel = QPushButton("Cancel")
    ...
    def _common_tab(self):
        layout = self._scroll_tab("General"); common = self.revision.common_parameters
        identity = QGroupBox("Event and source"); form = QFormLayout(identity)
        ...
        form.addRow("Source CSV", QLabel(common.source_csv or "—")); layout.addWidget(identity)
        block = QGroupBox("Block parameters"); form = QFormLayout(block)
        ...
        form.addRow("Block name", self.block_name); form.addRow("Working horizon", self.horizon); form.addRow("Comments", self.comments); layout.addWidget(block)
        if self.revision.production_parameters:
            p = self.revision.production_parameters; calc = QGroupBox("Calculated values"); f = QFormLayout(calc)
            f.addRow("Drilling area", QLabel(f"{p.drilling_area_m2.accepted_value:g} m²" if p.drilling_area_m2.accepted_value is not None else "— m²"))
            self.bench_height = _number(p.design_bench_height_m, "m"); self.explosive = _number(p.total_explosive_mass_kg, "kg")
            f.addRow("Design bench height", self.bench_height); f.addRow("Explosive mass", self.explosive); layout.addWidget(calc)
        else:
    ...
    def _geomechanics_tab(self):
        layout = self._scroll_tab("Geomechanics"); geo = self.revision.geomechanical_parameters
        identity = QGroupBox("Rock and geotechnical context"); form = QFormLayout(identity)
        self.lithology = QLineEdit(geo.lithology); self.geotechnical_domain = QLineEdit(geo.geotechnical_domain)
        form.addRow("Lithology", self.lithology); form.addRow("Geotechnical domain", self.geotechnical_domain); layout.addWidget(identity)
        strength = QGroupBox("Rock strength"); form = QFormLayout(strength)
        ...
        form.addRow("Local strength class", self.strength_class); form.addRow("Representative UCS", self.ucs); form.addRow("Minimum UCS",self.ucs_min); form.addRow("Maximum UCS",self.ucs_max); layout.addWidget(strength)
        quality = QGroupBox("Rock mass quality"); form = QFormLayout(quality)
        ...
        form.addRow("Representative RQD", self.rqd); form.addRow("Minimum RQD",self.rqd_min); form.addRow("Maximum RQD",self.rqd_max); form.addRow("Rock mass description", self.rock_properties); form.addRow("Fracturing",self.fracturing); form.addRow("Water conditions",self.water); form.addRow("Geomechanical notes",self.geo_notes); layout.addWidget(quality)
    ...
    def _render_groups(self):
        ...
        for group in self.revision.drilling_groups:
            ...
            charging = QGroupBox("Charging"); charging.setCheckable(True); charging.setChecked(False); charge_form = QFormLayout(charging)
            ...
            actions = QHBoxLayout(); duplicate = QPushButton("Duplicate"); remove = QPushButton("Delete")
    ...
    def _actual_tab(self):
        layout = self._scroll_tab("Execution fact"); execution = QGroupBox("A. Completion status"); form = QFormLayout(execution)
        self.completion_status = QComboBox()
        for key, name in (("planned","Planned"),("drilling","Drilling"),("charged","Charged"),("blasted","Blasted"),("completed","Completed"),("rejected","Rejected")): self.completion_status.addItem(name,key)
        actual=self.revision.actual_execution; self.completion_status.setCurrentIndex(self.completion_status.findData(actual.completion_status)); form.addRow("Status", self.completion_status)
        ...
        form.addRow("Actual blast date (YYYY-MM-DD)",self.actual_date); form.addRow("General notes",self.execution_notes); layout.addWidget(execution)
        summary=QGroupBox("B. Actual summary"); grid=QFormLayout(summary); self.actual_summary_widgets={}
        ...
        controls=QHBoxLayout(); copy_all=QPushButton("Copy design to actual"); copy_all.setObjectName("copyProjectToActualButton")
        add=QPushButton("+ Add actual group"); copy_all.clicked.connect(self._copy_all_actual); add.clicked.connect(self._add_actual_group)
        controls.addWidget(copy_all); controls.addWidget(add); controls.addStretch(); layout.addLayout(controls)
        section=QGroupBox("C. Actual drilling and charging groups"); self.actual_cards_layout=QVBoxLayout(section); layout.addWidget(section)
        comparison=QGroupBox("D. Design / actual comparison"); comparison_layout=QVBoxLayout(comparison)
    ...
    def _render_actual_groups(self):
        ...
        for group in self.revision.actual_execution.actual_drilling_groups:
            ...
            outer.addWidget(drilling); charging=QGroupBox("Charging"); cf=QFormLayout(charging)
            ...
            outer.addWidget(charging); deviation=QGroupBox("Deviations"); df=QFormLayout(deviation)
            ...
            outer.addWidget(deviation); actions=QHBoxLayout(); copy=QPushButton("Copy from design"); duplicate=QPushButton("Duplicate"); remove=QPushButton("Delete")
    ...
    def _copy_all_actual(self):
        ...
        if not actual.actual_drilling_groups:
            if QMessageBox.question(self,"Copy design","Create an independent snapshot of design groups in actuals?") != QMessageBox.StandardButton.Yes: return
            mode="replace"
        else:
            labels=["Fill empty fields only","Add missing groups","Replace actuals with design","Cancel"]
            choice,ok=QInputDialog.getItem(self,"Copy design","Choose a safe copy mode:",labels,0,False)
            if not ok or choice=="Cancel": return
            mode={labels[0]:"fill_empty",labels[1]:"add_missing",labels[2]:"replace"}[choice]
            if mode=="replace" and QMessageBox.warning(self,"Replace actuals","All entered actual values will be replaced with design values.",QMessageBox.StandardButton.Ok|QMessageBox.StandardButton.Cancel,QMessageBox.StandardButton.Cancel)!=QMessageBox.StandardButton.Ok: return
    ...
    def _copy_one_actual(self, design, actual):
        ...
        choice,ok=QInputDialog.getItem(self,"Copy from design","Actual values may already be populated:",labels,0,False)
    ...
    def _history_tab(self):
        layout = self._scroll_tab("Revision history"); table = QTableWidget(len(self.card.revisions), 5); table.setHorizontalHeaderLabels(["№", "Date", "Status", "Geometry revision", "Reason"])
