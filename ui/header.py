...
class Header(QWidget):
    add_mine_requested=Signal(); add_domain_requested=Signal(); add_block_requested=Signal(); add_assessment_area_requested=Signal()
    def __init__(self, context):
        ...
        self.add_button.setMenu(self.add_menu); self.search=QLineEdit(); self.search.setPlaceholderText("Search (Ctrl+F)"); self.search.setMaximumWidth(350); self.settings=QPushButton("Settings"); self.settings.clicked.connect(self.open_settings)
        layout.addWidget(self.add_button); layout.addStretch(); layout.addWidget(self.search); layout.addStretch(); layout.addWidget(self.settings); self.update_add_availability(False,False,False)
