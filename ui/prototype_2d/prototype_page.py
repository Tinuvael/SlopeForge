...
class Prototype2DPage(QWidget):
    ...
    def _create_controls(self) -> None:
        ...
        self.edit_button = QPushButton("Редактировать"); self.edit_button.clicked.connect(self.start_editing)
    ...
    def build_layout(self) -> None:
        ...
        info = QGroupBox("Информация"); info_layout = QVBoxLayout(info); info_layout.addWidget(self.line_info); info_layout.addWidget(self.pit_boundary_button); panel_layout.addWidget(info)
        ...
        bench_layout.addWidget(QLabel("Промежуточные сегменты")); bench_layout.addWidget(self.intermediate_list); bench_layout.addWidget(self.edit_intermediate_button); bench_layout.addWidget(self.delete_intermediate_button)
    ...
    def current_occupied_intervals(self):
        ...
        return occupied_intervals(self.state, self.state.active_dataset_id, line.source_id) if line else []
    ...
    def on_draft_selected(self, row: int) -> None:
        ...
        self.draft_details.setText(f"{draft.id}\nСтатус: {status}\nDataset: {dataset or '—'}\nНижняя граница: SID {lower.source_line_id if lower else '—'}, Z {lower.elevation if lower else '—'}, {segment_length(lower) if lower else 0:.1f} м\nВерхняя граница: SID {upper.source_line_id if upper else '—'}, Z {upper.elevation if upper else '—'}, {segment_length(upper) if upper else 0:.1f} м\nПромежуточные сегменты: {len(draft.intermediate_segment_ids)}")
        self.refresh_scene()
    ...
    def edit_selected_intermediate(self) -> None:
        ...
        self.state.segments = [item for item in self.state.segments if item.id != segment_id]
        self.controller.start_intermediate(bench.id)
        self.controller.choose_line(segment.source_line_id); self.controller.use_selected_line()
        self.controller.add_marker(segment.source_line_id, segment.start_position)
        self.controller.add_marker(segment.source_line_id, segment.end_position)
        bench.status = "editing"; self.update_workflow_panel(); self.refresh_scene()
