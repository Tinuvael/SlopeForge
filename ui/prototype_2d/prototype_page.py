...
from PySide6.QtWidgets import (
    ...
    QSplitter, QTextEdit, QToolButton, QVBoxLayout, QWidget, QAbstractSpinBox,
)
...
class Prototype2DPage(QWidget):
    ...
    def on_line_clicked(self, line_id: str, x: float, y: float) -> None:
        try:
            if self.controller.state in LINE_SELECTION_STATES:
                ...
                if not self.highlighted_bench_id:
    ...
    def on_marker_dragged(self, marker: str, x: float, y: float) -> None:
        ...
        self.scene.show_segment_preview(line, draft.start_position, draft.end_position)
        self.update_workflow_panel()
    ...
    def on_draft_selected(self, row: int) -> None:
        if row < 0: return
        draft = self.state.drafts[row]; self.controller.selected_bench_id = draft.id; self.highlighted_bench_id = draft.id; self.pit_boundary_button.setVisible(False)
    ...
    def selected_intermediate(self) -> LineSegmentSelection | None:
        ...
        if not bench or row < 0 or row >= len(bench.intermediate_segment_ids): return None
