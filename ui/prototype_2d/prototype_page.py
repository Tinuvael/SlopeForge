...
from .bench_list_panel import BenchListPanel
from .bench_inspector import BenchInspector
...
class Prototype2DPage(QWidget):
    ...
    def _create_controls(self) -> None:
        ...
        self.bench_list_panel = BenchListPanel()
        self.bench_list_panel.bench_selected.connect(self.on_draft_selected)
        self.bench_list_panel.add_requested.connect(self.start_bench_workflow)
        self.inspector = BenchInspector()
        self.inspector.edit_lower_button.clicked.connect(lambda: self.edit_boundary("lower"))
        self.inspector.edit_upper_button.clicked.connect(lambda: self.edit_boundary("upper"))
        self.inspector.add_intermediate_button.clicked.connect(self.start_intermediate_workflow)
        self.inspector.edit_intermediate_button.clicked.connect(self.edit_selected_intermediate)
        self.inspector.delete_intermediate_button.clicked.connect(self.delete_selected_intermediate)
        self.inspector.delete_bench_button.clicked.connect(self.delete_selected_bench)
        self.inspector.clear_selection_button.clicked.connect(self.clear_selection)
        self.workflow_panel = WorkflowPanel()
        self.workflow_panel.primary_button.clicked.connect(self.primary_action)
        self.workflow_panel.back_button.clicked.connect(self.go_back)
        self.workflow_panel.cancel_button.clicked.connect(self.cancel_workflow)
        self.workflow_step = self.workflow_panel.step
        self.workflow_instruction = self.workflow_panel.instruction
        self.workflow_selected = self.workflow_panel.summary
        self.primary_button = self.workflow_panel.primary_button
        self.back_button = self.workflow_panel.back_button
        self.cancel_button = self.workflow_panel.cancel_button
        self.retry_button = QPushButton()
        # Aliases keep event handlers compact while panels own the widgets.
        self.draft_list = self.bench_list_panel.list_widget
        self.draft_details = self.inspector.details
        self.intermediate_list = self.inspector.intermediate_list
        self.edit_lower_button = self.inspector.edit_lower_button
        self.edit_upper_button = self.inspector.edit_upper_button
        self.edit_intermediate_button = self.inspector.edit_intermediate_button
        self.delete_intermediate_button = self.inspector.delete_intermediate_button
        self.add_intermediate_button = self.inspector.add_intermediate_button
        self.delete_bench_button = self.inspector.delete_bench_button
        self.clear_selection_button = self.inspector.clear_selection_button
        self.finish_button = QPushButton("Готово")
        self.cancel_edit_button = QPushButton("Отменить изменения")
        self.finish_button.clicked.connect(self.finish_editing); self.cancel_edit_button.clicked.connect(self.cancel_editing)
        self.inspector.card.layout().addWidget(self.finish_button); self.inspector.card.layout().addWidget(self.cancel_edit_button)


    def build_layout(self) -> None:
        root = QVBoxLayout(self)
        top = QHBoxLayout()
        for text, callback in [("Импорт", self.import_csv), ("Сохранить", self.save)]:
        ...
        top.addWidget(fit); top.addWidget(QLabel("Рабочий горизонт")); top.addWidget(self.horizon_combo); top.addWidget(self.only_horizon_check); top.addWidget(self.grid_check); top.addStretch(1); top.addWidget(self.coord_label)
        root.addLayout(top)
        self.main_splitter = QSplitter(Qt.Orientation.Horizontal)
        self.main_splitter.addWidget(self.bench_list_panel)
        self.main_splitter.addWidget(self.view)
        self.main_splitter.addWidget(self.inspector)
        self.main_splitter.setStretchFactor(0, 1); self.main_splitter.setStretchFactor(1, 4); self.main_splitter.setStretchFactor(2, 1)
        self.main_splitter.setSizes([250, 900, 300])
        root.addWidget(self.main_splitter, 1)
        root.addWidget(self.workflow_panel)
    ...
    def clear_selection(self) -> None:
        ...
        self.inspector.set_selected(False); self.draft_details.clear(); self.refresh_scene()
    ...
    def refresh_lists(self) -> None:
        ...
        self.bench_list_panel.set_project("SlopeForge", self.state.active_dataset_id)
    ...
    def create_bench_with_checks(self) -> None:
        ...
        bench.status = "ready"
        self.refresh_lists()
        self.draft_list.setCurrentRow(len(self.state.drafts) - 1)
    ...
    def confirm_segment(self) -> None:
        ...
        if segment.role == "intermediate_assessment":
            self.controller.add_intermediate_to_bench(self.state)
            self.refresh_lists()
            self.controller.reset()
            self.update_workflow_panel(); self.refresh_scene()
    ...
    def on_draft_selected(self, row: int) -> None:
        ...
        self.inspector.set_selected(True)
        self.controller.selected_bench_id = draft.id; self.highlighted_bench_id = None; self.controller.lower.reset_points(); self.controller.upper.reset_points(); self.controller.intermediate.reset_points()
        ...
        self.inspector.upper_assessment_label.setVisible(draft.upper_boundary_is_assessment)
