...
class Prototype2DPage(QWidget):
    ...
    def _highlighted_segments(self) -> list[LineSegmentSelection]:
        # При добавлении/редактировании промежуточного сегмента границы уступа остаются контекстом.
        visible = self.bench_segments(self.controller.selected_bench_id or self.highlighted_bench_id)
        ...
        return visible + [segment for segment in pending if segment.id not in {item.id for item in visible}]
    ...
    def on_draft_selected(self, row: int) -> None:
        ...
        self.controller.selected_bench_id = draft.id; self.highlighted_bench_id = draft.id; self.pit_boundary_button.setVisible(False)
