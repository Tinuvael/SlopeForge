...
class PrototypePlanScene(QGraphicsScene):
    ...
    def set_lines(self, lines: list[DatamineLine], active_elevation: float | None = None, only_active_horizon: bool = False) -> None:
        self.clear(); self._horizon_label = None; self._items.clear(); self._segment_items.clear(); self._preview_items.clear(); self._marker_items.clear(); self._connection_items.clear()
        self._lines = lines
    ...
    def show_segment_preview(self, line: DatamineLine, start_position: float | None, end_position: float | None) -> None:
        self.clear_preview()
        ...
        for point in points[1:]:
            path.lineTo(point.x, -point.y)
        item = QGraphicsPathItem(path); item.setPen(QPen(QColor(255, 130, 0), 6)); item.setZValue(70); self.addItem(item); self._preview_items.append(item)
