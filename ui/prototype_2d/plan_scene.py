    ...
    def show_segments(self, segments: list[LineSegmentSelection], selected_segment_id: str | None = None) -> None:
        ...
        for segment in segments:
            ...
            for point in segment.extracted_points[1:]:
                path.lineTo(point.x, -point.y)
            item = QGraphicsPathItem(path); item.setPen(QPen(QColor(255, 130, 0) if segment.id == selected_segment_id else colors.get(segment.role, QColor(0, 120, 255)), 7 if segment.id == selected_segment_id else 5)); item.setZValue(60 if segment.id == selected_segment_id else 50); self.addItem(item); self._segment_items.append(item)
