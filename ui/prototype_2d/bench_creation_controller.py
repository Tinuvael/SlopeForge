...
class BenchCreationController:
    ...
    edit: SegmentDraft = field(default_factory=lambda: SegmentDraft("intermediate_assessment"))
    candidate_line_ids: list[str] = field(default_factory=list)
    candidate_index: int = 0
    candidate_anchor_x: float | None = None
    candidate_anchor_y: float | None = None
    ...
    def reset(self) -> None:
        ...
        self.edited_segment_id = None
        self.edit = SegmentDraft("intermediate_assessment")
    ...
    def current_draft(self) -> SegmentDraft | None:
        ...
        if self.state == BenchWorkflowState.EDIT_SEGMENT_CONFIRM:
            return self.edit
        return None

    def start_segment_edit(self, segment: LineSegmentSelection) -> None:
        self.edited_segment_id = segment.id
        self.edit = SegmentDraft(segment.role, segment.source_line_id, segment.start_position, segment.end_position)
        self.active_line_id = segment.source_line_id
        self.selected_line_id = segment.source_line_id
        self.state = BenchWorkflowState.EDIT_SEGMENT_CONFIRM

    def confirm_current_step(self) -> str | None:
        """Возвращает одно допустимое подтверждающее действие для Enter."""
        if self.state in {BenchWorkflowState.SELECT_LOWER_LINE, BenchWorkflowState.SELECT_UPPER_LINE, BenchWorkflowState.ADD_INTERMEDIATE_SELECT_LINE}:
            return "use_line" if self.selected_line_id else None
        if self.state in {BenchWorkflowState.CONFIRM_LOWER, BenchWorkflowState.CONFIRM_UPPER, BenchWorkflowState.ADD_INTERMEDIATE_CONFIRM}:
            return "confirm_segment"
        if self.state == BenchWorkflowState.CONFIRM_BENCH:
            return "create_bench"
        if self.state == BenchWorkflowState.BENCH_CREATED:
            return "finish"
        if self.state == BenchWorkflowState.EDIT_SEGMENT_CONFIRM:
            return "save_segment_edit"
        return None

    def set_line_candidates(self, ids: list[str], x: float, y: float) -> None:
        self.candidate_line_ids = ids
        self.candidate_index = 0
        self.candidate_anchor_x, self.candidate_anchor_y = x, y
        self.selected_line_id = self.current_candidate()

    def current_candidate(self) -> str | None:
        return self.candidate_line_ids[self.candidate_index] if self.candidate_line_ids else None

    def next_candidate(self) -> str | None:
        if self.candidate_line_ids:
            self.candidate_index = (self.candidate_index + 1) % len(self.candidate_line_ids)
            self.selected_line_id = self.current_candidate()
        return self.selected_line_id

    def previous_candidate(self) -> str | None:
        if self.candidate_line_ids:
            self.candidate_index = (self.candidate_index - 1) % len(self.candidate_line_ids)
            self.selected_line_id = self.current_candidate()
        return self.selected_line_id
    ...
    def clear_candidates(self) -> None:
        ...
        self.candidate_index = 0
        self.candidate_anchor_x = self.candidate_anchor_y = None
    ...
    def build_current_segment(self, state: PrototypeState, line: DatamineLine) -> LineSegmentSelection:
        ...
        segment_id = self.edited_segment_id or f"S-{len(state.segments) + confirmed + 1:03d}"
        dataset_id = next((item.dataset_id for item in state.segments if item.id == segment_id), state.active_dataset_id)
        return LineSegmentSelection(segment_id, line.source_id, start, end, points, draft.role, line.elevation, dataset_id=dataset_id)
    ...
    def create_bench(self, state: PrototypeState) -> BenchSectionDraft:
        ...
        bench = BenchSectionDraft(draft_id, self.upper.segment.id, self.lower.segment.id, [], draft_id, status="editing")
