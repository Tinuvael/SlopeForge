...
class AssessmentAreaCreationPage(QWidget):
    ...
    def __init__(self,context,domain_id,domain_name,site_id,parent=None,edit_area_id=None):
        ...
        bar=QHBoxLayout(); fit=QPushButton(tr("Fit")); fit.clicked.connect(self.controller.workspace.plan_view_fit); self.lines=QCheckBox(tr("Project Lines")); self.lines.setChecked(self.controller.workspace.lines_checkbox.isChecked()); self.lines.toggled.connect(self._toggle_lines); self.grid=QCheckBox(tr("Grid")); self.grid.setChecked(self.controller.workspace.grid_button.isChecked()); self.grid.toggled.connect(self._toggle_grid); self.start=QPushButton(tr("Draw boundaries")); self.start.clicked.connect(self._start_drawing); self.back_vertex=QPushButton(tr("Undo vertex")); self.back_vertex.clicked.connect(lambda:self._workflow_action("back")); self.finish=QPushButton(tr("Finish polygon / Continue")); self.finish.clicked.connect(self._continue); self.confirm=QPushButton(tr("Confirm boundaries")); self.confirm.clicked.connect(self._confirm); self.cancel_drawing=QPushButton(tr("Cancel drawing")); self.cancel_drawing.clicked.connect(self._cancel_drawing); close=QPushButton(tr("Back / Close")); close.clicked.connect(self._close_page)
    ...
    def _start_drawing(self):
        ...
        except Exception as exc: QMessageBox.critical(self,tr("Assessment Area"),f"Could not start boundary editing.\n\n{domain_message(str(exc))}")
        self._sync_status()
    ...
    def _continue(self):
        ...
        except Exception as exc:QMessageBox.critical(self,tr("Assessment Area"),f"Could not start boundary refinement.\n\n{domain_message(str(exc))}")
        self._sync_status()
    def _confirm(self):
        if self._completion_emitted:return
        try:self.controller.workspace.confirm_refined_polygon()
        except Exception as exc:QMessageBox.critical(self,tr("Assessment Area"),f"Could not save the new boundaries.\n\n{domain_message(str(exc))}")
        self._sync_status()
