...
class ContourEventPage(QWidget):
    ...
    def __init__(self,context,domain_id,domain_name,event_id,parent=None):
        ...
        card,draft=self.controller.technical_card_draft(self.blast_event); self.card,self.draft=card,draft; self.editor=TechnicalCardEditorWidget(self.blast_event,card,draft,self.controller.save_technical_card,self,self.read_only,products=create_explosive_catalogue(context).list_enabled_products(),preset_service=create_charge_presets(context),site_id=self.controller.site_id)
