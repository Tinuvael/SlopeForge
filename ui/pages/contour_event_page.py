...
"""Focused Block-style page for one contour BlastEvent."""
...
class ContourEventPage(QWidget):
    ...
    def _refresh_workflow_presentation(self):
        state=blast_workflow_for(self.controller.state,self.blast_event); workflow=tr(WORKFLOW_LABELS[state]); planned=self.blast_event.event_date or "—"
        self.header.set_content(title=f"{tr('Contour blast')} {self.blast_event.name}",status_text=workflow,status_state=state,archived=self.blast_event.is_archived,can_edit=not self.read_only,
            meta_values=(f"{tr('ID')}: {self.blast_event.id}",f"{tr('Horizon')}: {self.blast_event.elevation:g} m",f"{tr('Domain')}: {self.domain_name}",f"{tr('Planned blast date')}: {planned}",f"{tr('Revision')}: {self.rev.revision_number if self.rev else '—'}"))
        if hasattr(self,"general_information"):
            self.general_information["Status"].setText(workflow); self.general_information["Planned blast date"].setText(str(planned))
    def _sidebar(self,body):
        right=QVBoxLayout(); self.summary=CardFrame("Summary"); self.summary_grid=QGridLayout(); self.summary.layout.addLayout(self.summary_grid); right.addWidget(self.summary); self.photo_preview=QuickAttachmentPreview("Photos","photo"); self.document_preview=QuickAttachmentPreview("Documents","document"); right.addWidget(self.photo_preview); right.addWidget(self.document_preview); right.addStretch(); body.addLayout(right,1)
    def _general(self):
    ...
    def _refresh_sidebar(self):
        ...
        self.photo_preview.set_items(self.controller.attachments,photos,"No photos yet",can_add=not self.read_only); self.document_preview.set_items(self.controller.attachments,documents,"No documents yet",can_add=not self.read_only)
