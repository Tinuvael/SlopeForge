...
class ContourEventPage(QWidget):
    ...
    def __init__(self,context,domain_id,domain_name,event_id,parent=None):
        ...
        self._general(); self.tabs.addTab(BlastDesignEditorWidget(self.editor.take_tab(tr("Contour drilling"))),tr("Blast design")); self.tabs.addTab(ActualExecutionEditorWidget(self.editor.take_tab(tr("Execution fact"))),tr("Execution fact")); self.photos_tab=self._attachments("Photos"); self.documents_tab=self._attachments("Documents"); self.tabs.addTab(self.photos_tab,tr("Photos")); self.tabs.addTab(self.documents_tab,tr("Documents")); self.tabs.addTab(self.editor.take_tab(tr("Revision history")),tr("History")); self.tabs.currentChanged.connect(self._sync_engineering_actions_visibility); self._sync_engineering_actions_visibility(); self._refresh_sidebar()
    ...
    def _attachments(self,title):
        kind="photo" if title=="Photos" else "document"; page,manager=create_attachment_tab_page(self.controller.attachments,"blast_event",self.blast_event.id,kind,read_only=self.read_only); manager.changed.connect(self._refresh_sidebar); return page
