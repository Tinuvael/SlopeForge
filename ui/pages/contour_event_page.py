...
class ContourEventPage(QWidget):
    def __init__(self,context,domain_id,domain_name,event_id,parent=None):
        super().__init__(parent); self.context=context; self.controller=EntityPageController(context,domain_id); self.event=next(e for e in self.controller.state.blast_events if e.id==event_id and e.event_type=="contour"); self.read_only=not context.current_user.can_edit or self.event.is_archived
        ...
        card,draft=self.controller.technical_card_draft(self.event); self.editor=TechnicalCardEditorWidget(self.event,card,draft,self.controller.save_technical_card,self,self.read_only); layout.addWidget(self.editor.take_tab("Общие"))
    ...
    def _attachments(self,title):
        ...
        def open_dialog():
            ...
            EntityAttachmentDialog(self.controller.attachments,"blast_event",self.event.id,self,read_only=self.read_only).exec()
