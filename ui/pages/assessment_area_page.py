...
class AssessmentAreaPage(QWidget):
    ...
    def _build_editor(self):
        ...
        if not self.read_only and not self.evaluation_editor.inspector.text().strip(): self.evaluation_editor.inspector.setText(getattr(context.current_user,"display_name","") or "")
        metadata=QHBoxLayout(); metadata.addWidget(QLabel(tr("Assessment date"))); metadata.addWidget(self.evaluation_editor.date); metadata.addSpacing(12); metadata.addWidget(QLabel(tr("Inspector"))); metadata.addWidget(self.evaluation_editor.inspector,1); inputs.addLayout(metadata)
        ...
        context=QFrame(); context.setObjectName("CriterionCard"); context_form=QFormLayout(context); context_form.setContentsMargins(10,8,10,8); context_form.addRow(tr("Matrix"),self.evaluation_editor.matrix_value); context_form.addRow(tr("Detection"),self.evaluation_editor.detected)
