...
class DomainDashboardPage(QWidget):
    ...
    def _overview(self):
        ...
        problems=[(a.name,a.interval,metric(a.dai),metric(a.fci),a.assessment_date or "—",quadrant_presentation(a.quadrant)[0]) for a in self.snapshot.areas if a.status=="completed" and a.quadrant][:6]; table=self._table(["Area","Interval","DAI","FCI","Date","Result"],problems) if problems else EmptyStateWidget("No completed evaluations yet"); box.addWidget(section("Areas requiring attention",table)); box.addWidget(section("Recent changes",self._table(["Record","Changed"],self.snapshot.recent) if self.snapshot.recent else EmptyStateWidget("No recent activity"))); box.addWidget(section("Plan overview",DashboardPlanOverviewWidget(self.snapshot))); scroll.setWidget(body); return scroll
    def _blasts(self):
    ...
    def _areas(self):
        rows=[(x.name,x.interval,x.assessment_date or "—",x.status or "—",metric(x.dai),metric(x.fci),quadrant_presentation(x.quadrant)[0]) for x in self.snapshot.areas]; t=self._table(["Area","Interval","Assessment date","Status","DAI","FCI","Result"],rows); t.cellDoubleClicked.connect(lambda r,_:self.assessment_area_requested.emit(self.snapshot.areas[r].id)); return t
    def _analytics(self):
