...
from infrastructure.services.production_blast_service import ProductionBlastInput,PermissionDenied,ValidationError
...
class BlockDialog(QDialog):
    ...
    def __init__(self, service, domain_repo, user, domain_id=None, block=None,
        ...
        self.service=service; self.domain_repo=domain_repo; self.user=user; self.block=block
        self.saved_block_id=None; self.read_only=read_only or not user.can_edit
        self.setWindowTitle(tr("Edit block")); layout=QVBoxLayout(self); form=QFormLayout()
        self.domain=QComboBox(); self.block_number=QLineEdit(); self.horizon=QLineEdit()
        selected=block.domain_id; domain=domain_repo.get(selected)
        if domain:
        ...
        self.block_number.setText(block.block_number); self.horizon.setText(str(block.horizon_m))
    ...
    def _input(self):
        ...
        return ProductionBlastInput(domain_id,self.block_number.text(),self.horizon.text(),self.block.comment)
    ...
    def _save(self):
        try:
            ...
            self.saved_block_id=self.service.update_metadata(
                ...
                horizon_text=data.horizon_text,user=self.user,expected_version=self.expected_version,
