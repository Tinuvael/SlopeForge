...
class BlockDialog(QDialog):
    def __init__(self, service: BlastBlockService, mine_repo: MineRepository, site_repo: SiteRepository, user: CurrentUser, block: BlastBlockRow | None = None, read_only: bool = False):
        super().__init__()
        self.service = service; self.domain_repo = DomainRepository(service.session_factory); self.mine_repo = mine_repo; self.site_repo = site_repo; self.user = user; self.block = block; self.saved_block_id = None
    ...
    def _general_tab(self) -> QWidget:
        ...
        self.block_number = QLineEdit(); self.mine = QComboBox(); self.site = QComboBox(); self.domain = QComboBox(); self.horizon = QLineEdit()
        ...
        self.site.currentIndexChanged.connect(self._load_domains)
        form.addRow("Block number *", self.block_number); form.addRow("Mine *", self.mine); form.addRow("Site *", self.site); form.addRow("Domain *", self.domain)
    ...
    def _load_domains(self) -> None:
        site_id = self.site.currentData(); self.domain.clear()
        if site_id is None: return
        for domain in self.domain_repo.list_domains(site_id): self.domain.addItem(domain.name, domain.id)
    ...
    def _fill_from_block(self) -> None:
        ...
        site_idx = self.site.findData(self.block.site_id); self.site.setCurrentIndex(max(site_idx, 0)); self._load_domains(); domain_idx = self.domain.findData(self.block.domain_id); self.domain.setCurrentIndex(max(domain_idx, 0)); self.horizon.setText(str(self.block.horizon_m) if self.block.horizon_m is not None else "")
    ...
    def _set_read_only(self) -> None:
        for widget in [self.block_number, self.mine, self.site, self.domain, self.horizon, self.planned_date, self.status, self.comment]: widget.setEnabled(not self.read_only)
    ...
    def _input(self) -> BlastBlockInput:
        ...
        return BlastBlockInput(self.block_number.text(), self.mine.currentData(), self.site.currentData(), self.horizon.text(), planned, self.status.currentData(), self.comment.toPlainText(), self.domain.currentData())
