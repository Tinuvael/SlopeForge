...
class BlockDialog(QDialog):
    def __init__(self, service: BlastBlockService, mine_repo: MineRepository, site_repo: SiteRepository, user: CurrentUser, block: BlastBlockRow | None = None, read_only: bool = False):
        ...
        self.setWindowTitle("Карточка блока" if block else "Новый блок")
        self.resize(520, 430)
        ...
        form.addRow("Номер блока *", self.block_number); form.addRow("Месторождение *", self.mine); form.addRow("Участок *", self.site)
        form.addRow("Горизонт, м", self.horizon); form.addRow("Плановая дата", self.planned_date); form.addRow("Статус", self.status); form.addRow("Комментарий", self.comment)
        layout.addLayout(form)
        buttons = QHBoxLayout(); buttons.addStretch(); cancel = QPushButton("Закрыть" if self.read_only else "Отмена"); cancel.clicked.connect(self.reject); buttons.addWidget(cancel)
        self.save_button = QPushButton("Сохранить"); self.save_button.clicked.connect(self._save); self.save_button.setVisible(not self.read_only); buttons.addWidget(self.save_button); layout.addLayout(buttons)
    ...
    def _save(self) -> None:
        try:
            ...
            QMessageBox.warning(self, "Не удалось сохранить блок", str(exc))
